...
const Dashboard = () => {
  ...
  return (
    <div>
        ...
        {
          data?.map((item:any)=>{
            return <CameraListItem id={item.id} current_status={item.current_status} key={item.id} name={item.name} hasWarning={item.hasWarning} activeStatus={item.status} onActionClick={()=>deleteItem(item.id)} updateActiveStatus={updateActiveStatus} health={item.health} taskCount={item.tasks} recorderName={item.recorder} location={item.location}  />
          })
        }
